...
public class MazeJsonParser {
    ...
    /**
     * Read a game state from the current parser.
     * Json players do not include goal tiles, so they are arbitrarily assigned.
     */
    ...
    /**
     * Read a rotation count from the current parser. Assumes JSON uses counterclockwise rotations, so converts
     * to clockwise for use in Labyrinth methods.
     */
    ...
    /**
     * Read a Game from the current parser.
     * Players will be parsed according to whether or not they include goal tiles.
     */
    ...
    /**
     * Updates the goal position of a player.
     * Used when JSON data which constructs the player does not include the goal, but the goal is read later on.
     */
    ...
    /**
     * Read a list of players from the current parser.
     * If includeGoals is false, assign an arbitrary goal.
     */
    ...
    /**
     * Read a Player from the current parser.
     * If includeGoals is false, assign an arbitrary goal.
     */
    ...
    /**
     * Read a SlideAndInsert record from the current parser. Since it is reading a record used only for checking
     * reversal, JSON does not include rotations, so they are assigned to 0.
     */
