...
public class Game implements PrivateGameState {
  ...
  /**
   * Creates a new Game representing a game in progress with the given properties. The turn order is
   * defined by the Player list order. Throws an exception if a game is initialized with an illegal
   * Board or set of Players.
   */
  ...
  /**
   * Kicks the currently active Player by removing it from the list of Players. The next Player in
   * the list will become the active Player as its index has been reduced by 1. If the kicked Player
