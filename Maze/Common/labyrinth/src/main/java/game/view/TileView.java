...
//        this.drawGems(g);
    ...
    private void drawPathways(Graphics g) {
        ...
        if (this.tile.connects(UP)) {
            g.fillRect(this.getX() + this.cellSideLen / 2 - lineWidth / 2, this.getY() - offset, lineWidth, this.cellSideLen / 2 + lineWidth / 2);
        }
        if (this.tile.connects(DOWN)) {
            g.fillRect(this.getX() + this.cellSideLen / 2 - lineWidth / 2, this.getY() + this.cellSideLen / 2 - lineWidth / 2 - offset, lineWidth, this.cellSideLen / 2);
        }
        if (this.tile.connects(LEFT)) {
            g.fillRect(this.getX(), this.getY() + this.cellSideLen / 2 - lineWidth / 2 - offset, this.cellSideLen / 2, lineWidth);
        }
        if (this.tile.connects(RIGHT)) {
            g.fillRect(this.getX() + this.cellSideLen / 2, this.getY() + this.cellSideLen / 2 - lineWidth / 2 - offset, this.cellSideLen / 2, lineWidth);
        }
    }
    ...
    private void drawGems(Graphics g) {
        ...
        if (gems.size() != 2) {
            throw new IllegalStateException("Treasure does not have exactly two gems when trying to display.");
        }
        try {
            this.drawGem(g, gems.get(0).withDashes(), this.getX() + 10, this.getY() + 10);
            this.drawGem(g, gems.get(1).withDashes(), this.getX() + this.cellSideLen - 10 - this.imgSideLen,
